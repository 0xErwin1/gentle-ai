...
- [x] T1: Add six configurable role entries to custom Claude picker and preserve persisted assignments and generated agent fallback. Acceptance: picker can save/reopen all six roles; missing/invalid roles retain established default behavior. Checks: focused picker, app/state, generator tests passed. Route: delegated multi-file writer. Evidence: focused `go test` on picker, app, and Claude generator passed; five existing `review-*` agents map to saved roles, while validator remains native-process-only. Commit: pending.
- [ ] T2: Resolve saved role assignment at native RDD execution and pass model through isolated Claude process without changing isolation flags or other providers. Acceptance: all six roles select their configured model; absent/invalid assignments fall back as before; transport tests cover process arguments. Checks: focused reviewerprovider and cli tests. Route: delegated multi-file writer.
