...
A captured targeted validation whose evidence reports the immutable candidate could not be inspected carries no verdict in either direction, so it is neither admitted nor treated as unverifiable evidence. Routing answers `targeted_validation_inconclusive_recapture_required`: a `collect` transition carrying the same `external.run_targeted_validation` input and submission descriptor as `targeted_validation_required`. Nothing transitioned and the correction attempt was not consumed; restore the validator's read-only access to the frozen trees and run the same targeted validation again, then submit it through that descriptor. The already-occupied correction-bound validator slot is immutable and is never overwritten, which is why the recapture is submitted to FINALIZE directly instead of through `review capture-validation`. A captured validator result that is corrupt, partially published, non-canonical, or bound to another request remains `captured_artifacts_unverifiable`.
