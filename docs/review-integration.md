...
Claude Code is the only supported v2.1 immutable receipt-review transport. OpenCode and Codex are eligible but transport-disabled; Pi, Kilo, unknown, alias, and casing variants are ineligible. STATUS and START stop with `immutable_review_transport_unsupported` before repository, target, authority, or process work when the exact supported `--agent claude-code` identity is absent.
