...
No built-in runtime currently provides the enforced fresh reviewer-execution boundary. Claude prompt-carried context and OpenCode provider injection remain capture mechanisms, not eligibility claims. A generated v2 command that declares `--agent <runtime>` receives `immutable_review_transport_unsupported` before repository, target, authority, collection, or process work for every current built-in runtime. That typed result is terminal for the requested review; normal SDD and the manual compatibility path remain available.
