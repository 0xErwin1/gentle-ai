...
| `corrected_candidate_unavailable` | Change the candidate content so it differs from the frozen original, then re-run `gentle-ai review status --next-transition` (or `review finalize`) to receive the `targeted_validation` collection. See "After a correction forecast and an actual candidate change" above. |
