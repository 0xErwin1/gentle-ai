...
`gentle-ai review quarantine-legacy-fix-scope` is a separate maintenance operation for one legacy-v1 `ordinary_4r` lineage whose only replay failure is a `review/complete-fix` transition from `fixing` to `fix_validating` that adds one or more canonical correction paths outside its frozen genesis paths. It accepts neither aliases nor any other profile, state, operation, successor anomaly, scope change, or corruption. In particular, `repair-legacy-alias` remains limited to its alias-only class and refuses these lineages.

The maintainer authorization is an exact LF-only eight-line binding with schema `gentle-ai.review-legacy-fix-scope-quarantine-authorization/v1`, followed by `repository`, `lineage`, `revision`, `diagnostic`, `disposition`, `actor`, and `reason`. Actor and reason are non-empty single lines; the required diagnostic is `historical complete-fix expanded correction paths beyond frozen genesis scope`; the required disposition is `quarantine-historical-complete-fix-scope-expansion`. Repository binding is canonical, the legacy HEAD is an exact CAS value, and the authorization has no trailing newline.

Native replay rechecks every event payload hash and predecessor link, all valid transitions, the complete-fix event hash and recomputed fix-delta hash, and derives a non-empty expansion from the persisted path lists. It holds both the lineage lock and exclusive shared maintenance lock, refuses v1/v2 collisions, writes a prepared then committed quarantine record, moves the entire immutable lineage atomically, and reads inventory back. The local lock closes before the rename so the operation remains valid on Windows. Exact committed retries return the original audit record; partial moves retain the prepared audit record for reconciliation.
