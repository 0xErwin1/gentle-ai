...
func TestBoundedReviewContractRequiresRuntimeBoundReviewerContext(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Claude Code's lens agents expose no shell outside prompt-carried context",
		"OpenCode's lens agents expose neither shell nor a read tool",
		"Claude Code and OpenCode are the two supported immutable receipt-review runtimes",
		"`OPENCODE_DISABLE_PROJECT_CONFIG` and `OPENCODE_DISABLE_EXTERNAL_SKILLS` are both set",
		"Only then is the injected block provably its only byte source",
		"OpenCode also fetches any remote (http/https) `instructions` entry unconditionally",
		"the plugin separately reads the effective configuration through its own OpenCode client",
		"refuses to launch the reviewer if one is present, naming the offending entry",
		"Codex is eligible but transport-disabled",
