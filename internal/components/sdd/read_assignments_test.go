...
func TestReadCurrentModelAssignmentsNativeAgents(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{"agent":{"general":{"model":"openai/gpt-5"},"explore":{"model":"anthropic/claude-sonnet-4","variant":"high"}}}`), 0o644); err != nil { t.Fatal(err) }
	...
	if err != nil || len(custom) != 0 { t.Fatalf("native entries discovered as custom: %v, %v", custom, err) }
}
