...
5. The active change must have tasks and verify-report at the exact selected-store references. SDD never offers or launches RDD. Archive reads only task completion and independent verification; review mode does not change archive readiness. Proposal/spec/design are expected for full spec-driven archive; if missing, report the exact missing artifacts and require an explicit user override before archiving.
