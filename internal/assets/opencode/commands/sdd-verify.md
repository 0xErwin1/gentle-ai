...
TASK:
If all gates pass, launch the hidden `sdd-verify` sub-agent with the structured status, available artifacts, and strict TDD instructions if `sdd-init` detected strict TDD — there is no review-state prerequisite. This is the single independent requirements/runtime verification; a contradiction escalates and never starts another review/refuter/fix loop. After verify returns, rerun native SDD status: present the post-verify review offer only if the output contains a fresh `reviewOffer` block; if that block is absent, proceed toward archive with no review ceremony.
