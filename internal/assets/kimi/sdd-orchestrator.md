...
2. **Require effective authenticated advertisement**: treat native work routing as available only when the typed response uses the exact `gentle-ai.work-capabilities/v1` schema and contract, binds `agentId` to the current runtime identity, binds `repositoryRef` to the current repository, reports `workRouting.exposure` as `advertised`, advertises `contracts.start` as exactly `gentle-ai.work-start/v1` and `contracts.advance` as exactly `gentle-ai.work-advance/v1`, and contains a non-empty `connectorSessionRef`.
...
5. **Keep authority invisible**: retain the returned `workRunId`, `revision`, and typed provenance or authorization references internally for the post-actor advance, read-only status, and exact transition calls. In normal conversation, speak in the user's outcome language. Never expose handshake vocabulary or ask the user to choose or provide a route, agent, repository, policy, hash, nonce, issue, pull request, or delivery mechanism.
6. **Preserve legacy behavior**: if the capability response is dormant, unavailable, unauthenticated, malformed, unsupported, or omits either exact start or advance advertisement, do not call `work-start` and never infer support. Continue the existing legacy direct-inline, delegated-direct, and optional-SDD behavior without pretending that a managed WorkRun exists.
