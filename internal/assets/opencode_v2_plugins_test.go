...
const dir=path.join(root,'.gentle-ai','cache','opencode-v2');const read=async()=>JSON.parse(await fs.readFile(path.join(dir,await waitFor(async()=>(await fs.readdir(dir))[0],'cache entry')),'utf8'));
...
const second=await plugin.setup({location:{...location,workspaceID:'two'},model:ctx.model,event:{subscribe(opts){return {[Symbol.asyncIterator]:async function*(){await new Promise(r=>opts.signal.addEventListener('abort',r))}}}}});await waitFor(async()=>(await fs.readdir(dir)).length>=2,'second cache');
