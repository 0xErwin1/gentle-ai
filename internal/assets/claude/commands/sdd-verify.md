...
This is the single independent requirements/runtime final verification — it runs directly with no review-state prerequisite. A contradiction or failing check escalates; it never launches another review/refuter/fix loop. After verify passes, present the post-verify review offer only if the native status output contains a fresh `reviewOffer` block; if that block is absent, proceed toward archive with no review ceremony — do not call `gentle-ai review status` and do not fabricate an offer.
