...
If FINALIZE or the burn is ambiguous or fails, do not claim approval and do not ask the commit question. Query exact-lineage native STATUS only while that authority still exists, then follow the returned action. Never retry by inventing a new binding.
...
- After B is selected, retain canonical B as the lifecycle working root from selectorless STATUS through consent, collection, correction, validation, FINALIZE, burn, and the post-approval question. Do not fall back to A.
...
- The commit question names B and commits in B only if the human chooses it. Do not push.
...
| 05 | native single-select question | stop for the human's explicit commit authorization or uncommitted continuation |
...
Shipped `review validate` and gate commands are compatibility/informational only. They never discover authority or decide delivery: enabled gates return `invalidated/unmanaged`; disabled gates return `disabled/unmanaged`. They never allow, approve, block, commit, push, or open a PR.

Immediately after terminal `approved` burn, and before another edit or START, use the native question UI for one single-select question and STOP for the answer. When the review selected repository B from session A, name B in the question and run any chosen commit in B only:

- **Commit approved changes (Recommended)** — explicit authorization to create a conventional commit based on the approved work. Commit advances `HEAD`, so the next default START reviews only later changes. Do not push.
- **Continue uncommitted** — keep the workspace unchanged. The next START freezes the full outstanding delta again, including the approved bytes.

Push and PR are separate explicit decisions. Approval never authorizes delivery. If approved bytes are committed, the next default review starts from the new `HEAD`; otherwise it freezes the entire outstanding worktree delta.
