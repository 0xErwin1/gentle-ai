...
// A packages entry naming a Pi package the adapter does not already install
// must reach the rendered manifest as an additional "pi install" command,
// appended after the fixed six, without changing their order or count.
func TestRenderManifestInstallsAnExtraPiPackage(t *testing.T) {
	document := `{"version":"v1","selection":{"agents":["pi"],"providers":{"pi":{"packages":{"my-plugin":"git:github.com/x/y@rev"}}}}}`
	configPath := filepath.Join(t.TempDir(), "config.json")
	if err := os.WriteFile(configPath, []byte(document), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if len(commands) != 8 {
		t.Fatalf("Pi provisioning commands = %v, want 8 (the fixed six plus one extra): %s", commands, output)
	}
