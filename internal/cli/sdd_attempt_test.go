...
func TestRunSDDAttemptHelpContractsCoverEveryOperation(t *testing.T) {
	tests := []struct {
		...
		{"settle", []string{"cwd", "change", "token", "request-id", "outcome", "evidence-revision", "diagnosis", "harness-disposition", "cleanup-evidence", "process-evidence", "successor-lineage", "remediates-evidence-revision"}, []string{"opaque token returned by acquire", "empty or canonical legacy sha256 revision"}},
