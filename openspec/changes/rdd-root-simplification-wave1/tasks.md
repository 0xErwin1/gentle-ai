...
- [x] 5.3 RED: Off by Default in Live Paths — default config issues zero live Git cost from the shadow path. Same test as 5.2: `shadowObserverCallCountForTest()` proves the hook is reached while `shadowObserverRowsForTest()`/stderr stay empty — the suggested "test hook counter" proof.
