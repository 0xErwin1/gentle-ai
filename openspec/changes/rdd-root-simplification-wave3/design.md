...
- [ ] `provable_contraction` needs admitted-finding paths at gate time; the v3 record must persist admitted finding path references for the gate to avoid Amendment B's degradation-to-`changed` on every contraction. Confirm that field belongs in `review-state.json` rather than the receipt.
- [ ] Baselining the unwired offer API (decision 8) weakens the ratchet for one symbol for one wave. Confirm acceptable versus deferring the API to Wave 4.
