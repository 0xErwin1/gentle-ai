...
- [ ] 2.1 RED: `TestLegacyFunnelCharacterization_RunFacadeLegacyValidateNegotiated` — Wave-1 golden covering-array pattern (`-update`) pinning `runFacadeLegacyValidateNegotiated`'s observable contract (currently zero test references). **NOT STARTED** (deferred — genuine scope: subprocess re-entry, no reusable fixture infra found yet).
...
- [ ] 2.3 RED: `TestPrePRChainCompositionRemovalDelta` — DELTA rows layered onto existing `compact_chain_test.go`'s 25 test funcs, isolating exactly the rows S5 will delete. **NOT STARTED** (deferred — requires deep read of the existing 1101-line/25-func file before adding delta rows safely).
...
- [ ] 2.5 GREEN: 2.1–2.4 pass against current (pre-cutover) code — zero behavior change. **PARTIAL**: 2.2 and 2.4 pass (2/4); 2.1 and 2.3 not yet written.
