...
- [ ] 4.4 Wire the offer call site in `internal/cli`'s SDD verify-success exit (after verify-report admission, before archive eligibility) — `sddstatus.Resolve` stays a pure read. **BLOCKED — design gap found, not silently resolved**: the only existing verify-related `internal/cli` command, `RunSDDVerifyValidate` (`sdd_verify_validate.go`), is deliberately repo/context-free by its own doc comment ("validates a complete report without touching an artifact store") — it takes no `--cwd`/`--change`/`--lineage`, so it cannot call `OfferReviewAfterVerify(ctx, repo string, OfferRequest{LineageID})` without a new flag contract design.md does not specify. Additionally `internal/assets/*/commands/sdd-verify.md`'s own routing prose ("Continue only when the native bounded transaction is `ready_final_verification` or `final_verifying`") still encodes pre-verify review gating that design.md's File Changes table does not list for removal — a second, asset-level manifestation of the same pre-verify control this slice removes at the Go-code level. Needs maintainer/design clarification before wiring; see apply-progress.
- [ ] 4.5 Implement decline = nothing recorded: unmanaged proceed, `reviewGate.delivery: disabled/unmanaged`, byte-identical to kill-switch-OFF. Blocked on 4.4.
- [x] 4.6 GREEN: 4.1 passes (both packages). `.deadcode-baseline.txt` entry for `review_offer.go` intentionally NOT removed — it is still genuinely unwired (4.4 blocked), so removing the baseline entry now would be false.
- [ ] 4.7 Corroborating: OFF-mode bench call-absence counter — `reviewEntryHook`/`offerEntryHook` fire zero times across apply→verify→archive. Blocked on 4.4 (nothing to exercise end-to-end yet).
- [ ] 4.8 Integration test: decline output byte-identical to OFF golden. Blocked on 4.4/4.5.
- [x] 4.9 `scripts/deadcode-ratchet.sh --update` for door symbols and removed routing functions. Net-zero: 233/233 (hook `func(){}` var literals are not tracked as named unreachable functions by the ratchet's call-graph analysis).
