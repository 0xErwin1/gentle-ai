...
- [ ] 6.5 Modify `runtime_ledger.go`: `RuntimeStatus.Receipt *SDDReceiptRef`, new `runtimeReceiptEvent` (operation `receipt`), replacing `BindingRevision`/`Binding *ReviewBinding`. **DEFERRED — confirmed blast radius exceeds this slice's safe budget**: `codegraph_explore` shows `ReviewBinding` has 26+ callers (`review_binding.go`, `runtime_ledger.go`'s `bindPreparedReview` CAS/locking + legacy-import idempotency logic, `internal/cli/review_operation_contract_test.go`); `RuntimeStatus` itself has 18+ callers across `runtime_ledger.go`/`status.go`. Replacing these fields requires rewriting `bindPreparedReview`'s compare-and-swap and one-time legacy-import semantics, `runtimeBindingEvent`, and the `runtime_ledger_binding_test.go`/`runtime_ledger_remediation_test.go` suites — a materially larger, separate migration than tasks 6.1-6.4's additive new files. Not attempted without matching verification budget; see apply-progress for the full investigation.
- [ ] 6.6 Modify `review_gate.go`: `applyReviewGateEvaluation` keeps only the provider verdict; disabled/unmanaged branch becomes structurally unreachable when OFF. **DEFERRED** — depends on 6.5/6.7 landing first (this function currently branches on the pre-`SDDReceiptRef` `reviewAuthorityEvaluation` shape).
- [ ] 6.7 Collapse `resolveReviewAuthority` + `resolveCompactRemediationAuthority` into the single `ValidateSDDReceiptRef` call; `sddstatus` stores `result`/`reason` verbatim. **DEFERRED** — same blast-radius reason as 6.5; `resolveReviewAuthority` alone is 4 callers deep with receipt-discovery, ambiguous-governance, and stale-evidence branches `ValidateSDDReceiptRef`'s current additive shape does not yet replicate. **Consequence**: `OfferReviewAfterVerify`'s enabled-case `Available:false` cannot yet become a genuine composed decision — that composition needs 6.5-6.7's receipt data actually flowing through the runtime ledger, which this deferred, cannot happen in this slice.
