...
- [ ] WU8 (≤1000, measure reconcile-only subset): delete `ReconcileInvalidRecoveryEdge` (`compact_reconcile.go:233`; `review_reconcile.go:46` already gone with WU7)+reconcile-only subset of `compact_reconcile_test.go` (of 1192L total)+the 3 now-dead helpers WU7's own deadcode ratchet run confirmed have zero remaining callers besides `ReconcileInvalidRecoveryEdge` and its tests (`compactCombinedReconcileAuthorizationBinding`, `compactNonReconcilableContinuation`, `compactReconcileAuthorizationBinding`); split WU8/WU8b if subset pushes over budget. NOT STARTED — deferred, see apply-progress.
